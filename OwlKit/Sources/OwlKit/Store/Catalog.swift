...
// owl's catalog: one SQLite database per copy (owl.sqlite beside config.json)
...
// `owl` command share the file: write-ahead log, and a busy timeout for the
// moments both write. Nothing outside OwlKit sees SQL: Session.swift and
// Usage.swift are the ways in.
final class Catalog: @unchecked Sendable {
    static let shared = Catalog(Config.root.appending(path: "owl.sqlite"))

    private let queue = DispatchQueue(label: "owl.catalog")
