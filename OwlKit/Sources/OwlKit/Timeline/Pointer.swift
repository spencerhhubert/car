...
// The lines owl puts on the clipboard for pasting into an agent. Never the
// words: a line that names what to read, for an agent that knows owl (its
...
// over what was said up to it, and `owl marker` waits for those words to be
// transcribed. The session line is for handing over a whole session.
// The development copy writes owl-dev, so the agent reads the right catalog.
public enum Pointer {
    ...
    /// "new owl session 20260924-122534"
